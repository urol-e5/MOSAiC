...
if (nrow(its2_data) > 0) {
  ...
  # Convert all ITS2 type columns to numeric to avoid type conflicts
  its2_data_clean <- its2_data %>%
    mutate(across(all_of(its2_type_cols), ~as.numeric(as.character(.))))
  
  # Prepare data for plotting - select top abundant types for visualization
  plot_data <- its2_data_clean %>%
  ...
  # Create plot data
  timepoint_data <- its2_data_clean %>%

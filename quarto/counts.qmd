...
# Create sample count summary for visualization
sample_summary <- data.frame(
  Species = rep(c("Acropora pulchra", "Porites evermanni", "Pocillopora tuahiniensis"), each = 4),
  Feature_Type = rep(c("Gene", "miRNA", "lncRNA", "mCpG"), 3),
  Sample_Count = c(
    ncol(Apul_gene) - 1, ncol(Apul_mirna) - 1, ncol(Apul_lncRNA) - 1, ncol(Apul_mCpG) - 1,
    ncol(Peve_gene) - 1, ncol(Peve_mirna) - 1, ncol(Peve_lncRNA) - 1, ncol(Peve_mCpG) - 1,
    ncol(Ptuh_gene) - 1, ncol(Ptuh_mirna) - 1, ncol(Ptuh_lncRNA) - 1, ncol(Ptuh_mCpG) - 1
  )
)

# Convert to factors for better plotting
sample_summary$Species <- factor(sample_summary$Species, 
                                levels = c("Acropora pulchra", "Porites evermanni", "Pocillopora tuahiniensis"))
sample_summary$Feature_Type <- factor(sample_summary$Feature_Type, 
...
p1 <- ggplot(sample_summary, aes(x = Feature_Type, y = Sample_Count, fill = Species)) +
...
detect_outliers <- function(data_matrix, matrix_name) {
  # Calculate row sums (total counts per feature)
  feature_totals <- rowSums(data_matrix[, -1], na.rm = TRUE)
...
matrices_list <- list(
  ...
  "Apul_mCpG" = Apul_mCpG,
  "Peve_Gene" = Peve_gene,
  "Peve_miRNA" = Peve_mirna,
  "Peve_lncRNA" = Peve_lncRNA,
  "Peve_mCpG" = Peve_mCpG,
  "Ptuh_Gene" = Ptuh_gene,
  "Ptuh_miRNA" = Ptuh_mirna,
  "Ptuh_lncRNA" = Ptuh_lncRNA,
  "Ptuh_mCpG" = Ptuh_mCpG
)

# Perform outlier analysis for each matrix
outlier_results <- lapply(names(matrices_list), function(name) {
  detect_outliers(matrices_list[[name]], name)
})
names(outlier_results) <- names(matrices_list)

# Combine all summaries
all_outlier_summaries <- do.call(rbind, lapply(outlier_results, function(x) x$summary))
```
...
for (name in names(outlier_results)) {
  matrix_data <- data.frame(
    Matrix = name,
    Feature_Total = outlier_results[[name]]$feature_totals,
    Is_Outlier = seq_along(outlier_results[[name]]$feature_totals) %in% outlier_results[[name]]$outlier_indices
  )
...
plot_data$Species <- case_when(
  grepl("Apul", plot_data$Matrix) ~ "Acropora pulchra",
  grepl("Peve", plot_data$Matrix) ~ "Porites evermanni", 
  grepl("Ptuh", plot_data$Matrix) ~ "Pocillopora tuahiniensis"
)

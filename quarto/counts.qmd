...
```{r data-config, cache=TRUE, cache.lazy=FALSE, include=FALSE}

# Set timeout for downloads (120 seconds)
options(timeout = 120)
...
data_urls <- list(
  Apul = list(
    ...
    epimachine = "https://raw.githubusercontent.com/urol-e5/timeseries_molecular/refs/heads/main/D-Apul/output/36-Apul-epimachine-counts/Apul-epimachine-expression.csv",
    ...
    mCpG = "https://gannet.fish.washington.edu/metacarcinus/E5/20250903_meth_Apul/merged-WGBS-CpG-counts_filtered_n20.csv",
    gene_meth = "https://raw.githubusercontent.com/urol-e5/timeseries-molecular-calcification/refs/heads/main/D-Apul/output/40-Apul-Gene-Methylation/Apul-gene-methylation_75pct.tsv"
  ),
  Peve = list(
    ...
    mCpG = "https://gannet.fish.washington.edu/metacarcinus/E5/Pevermanni/20250821_meth_Peve/merged-WGBS-CpG-counts_filtered_n20.csv",
    gene_meth = "https://raw.githubusercontent.com/urol-e5/timeseries-molecular-calcification/refs/heads/main/E-Peve/output/15-Peve-Gene-Methylation/Peve-gene-methylation_75pct.tsv"
  ),
  Ptuh = list(
    ...
    mCpG = "https://gannet.fish.washington.edu/metacarcinus/E5/Ptuahiniensis/20250821_meth_Ptua/merged-WGBS-CpG-counts_filtered_n20.csv",
    gene_meth = "https://raw.githubusercontent.com/urol-e5/timeseries-molecular-calcification/refs/heads/main/F-Ptua/output/09-Ptua-Gene-Methylation/Ptua-gene-methylation_75pct.tsv"
  )
)
...
load_data_file <- function(url, data_type, species_code, max_retries = 3) {
  if (!validate_url(url)) {
  ...
  for (attempt in 1:max_retries) {
    result <- tryCatch({
      # Show progress
      if (attempt == 1) {
        message(paste("Loading", data_type, "for", species_code, "..."))
      } else {
        message(paste("  Retry attempt", attempt, "for", data_type, species_code))
      }
      
      # CSV-based
      if (data_type %in% c("gene", "mCpG", "epimachine")) {
        read_csv(url, show_col_types = FALSE, progress = FALSE)
      
      # TSV-based
      } else if (data_type %in% c("mirna", "lncRNA", "gene_meth")) {
        read_delim(url, delim = "\t", col_names = TRUE, show_col_types = FALSE, progress = FALSE)
      ...
      if (attempt < max_retries) {
        ...
        warning(paste("Failed to load", data_type, "data for", species_code, "after", max_retries, "attempts:", e$message))
        return(data.frame())
      }
    })
    
    # If we got a valid result, check and return it
    if (!is.null(result) && is.data.frame(result)) {
      if (nrow(result) == 0) {
        warning(paste("Empty data for", data_type, "in", species_code))
      } else {
        message(paste("  ✓ Loaded", nrow(result), "rows"))
      }
      return(result)
    }
  }
...
message("Starting data load for all species...")
...
for (species_code in names(data_urls)) {
  message(paste("\n===", data_urls[[species_code]]$species_name, "==="))
  all_data[[species_code]] <- list()
  
  for (data_type in c("gene", "epimachine", "mirna", "lncRNA", "mCpG", "gene_meth")) {
...
if (length(data_loading_errors) > 0) {
  warning("Data loading errors occurred:\n  ", paste(data_loading_errors, collapse = "\n  "))
} else {
...
for (species_code in names(data_urls)) {
  ...
  epimachine_count <- safe_nrow(all_data[[species_code]]$epimachine)
  ...
  gene_meth_count <- safe_nrow(all_data[[species_code]]$gene_meth)
  
  feature_counts <- rbind(feature_counts, data.frame(
    Species = data_urls[[species_code]]$italic_name,
    Gene = gene_count,
    Epimachine = epimachine_count,
    miRNA = mirna_count,
    lncRNA = lncrna_count,
    mCpG = mcpg_count,
    Gene_Meth = gene_meth_count,
    stringsAsFactors = FALSE
  ))
}
...
if (nrow(feature_counts_data) > 0) {
  feature_counts_data |>
    ...
    fmt_number(columns = c("Gene", "Epimachine", "miRNA", "lncRNA", "mCpG", "Gene_Meth"), decimals = 0) |>
    cols_align(align = "center", columns = c("Gene", "Epimachine", "miRNA", "lncRNA", "mCpG", "Gene_Meth")) |>
    tab_style(
      style = cell_text(weight = "bold"),
      locations = cells_body(columns = c("Gene", "Epimachine", "miRNA", "lncRNA", "mCpG", "Gene_Meth"))
    ) |>
    print()
} else {
...
Gene body methylation values are computed by overlapping EM-seq CpG methylation data with gene boundaries defined in the reference genome annotation. For each gene, all CpG sites falling within the gene body (transcription start to stop) are identified, and their methylation percentages are averaged across samples. Only genes with sufficient CpG coverage (≥75% of CpGs covered at 10× depth) are retained, ensuring robust methylation estimates. The resulting matrix provides a gene-level summary of methylation status, enabling integration with transcriptomic data for multi-omic analysis.
:::
  ...
  for (data_type in c("gene", "epimachine", "mirna", "lncRNA", "mCpG", "gene_meth")) {
    ...
    if (is.null(df) || nrow(df) == 0) {
      data_type_display <- switch(data_type,
        "gene"  = "Gene",
        "epimachine" = "Epigenetic Machinery",
        "mirna" = "miRNA",
        "lncRNA"= "lncRNA",
        "mCpG"  = "mCpG Percentage",
        "gene_meth" = "Gene Body Methylation",
        data_type
      )
    ...
    # File name
    file_extension <- if (data_type %in% c("gene","epimachine","mCpG","gene_meth")) ".csv" else ".txt"
